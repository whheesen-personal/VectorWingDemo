...
type Item = { id: string; group: string; start: Date; end: Date; title: string; content: string; status: 'Planned' | 'Authorized' | 'Canceled'; conflict?: boolean; notes?: string }
...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    timeline.current.on('itemmoved', (ev: any) => {
      setItems((prev) => markConflicts(prev.map((p) => (p.id === ev.item ? { ...p, start: ev.start, end: ev.end } : p))))
    })
  ...
  useEffect(() => {
    ...
    dsItems.current.add(visibleItems.map((it) => ({ ...it, className: `status-${it.status}${it.conflict ? ' conflict' : ''}` })) as any)
